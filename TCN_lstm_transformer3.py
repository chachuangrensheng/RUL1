...
class CrossAttention(nn.Module):
    ...
    def __init__(self, proj_dim=64, num_heads=4):
        ...
        self.kv_proj = nn.Linear(proj_dim, proj_dim * 2)  # 合并KV投影
    ...
    def forward(self, query_feat, key_feat):
        ...
        # 生成Q/K/V
        Q = self.q_proj(query_feat).view(batch_size, self.num_heads, self.head_dim)
        KV = self.kv_proj(key_feat).view(batch_size, 2, self.num_heads, self.head_dim)
        K, V = KV[:, 0], KV[:, 1]  # [B, H, D]

        # 注意力计算
        attn = (Q @ K.transpose(-2, -1)) * self.scale  # [B, H, 1, 1]
        attn = F.softmax(attn, dim=-1)
        output = (attn @ V).transpose(1, 2).reshape(batch_size, -1)
        return output


class LiteMLALayer(nn.Module):
    """轻量版多头潜在注意力（适配小hidden_size）"""

    def __init__(self, proj_dim=64, num_heads=4, latent_dim=16):
        super().__init__()
        self.num_heads = num_heads
        self.key_compress = nn.Linear(proj_dim*2, latent_dim * num_heads)
        self.value_compress = nn.Linear(proj_dim*2, latent_dim * num_heads)
        self.out_proj = nn.Linear(latent_dim * num_heads, proj_dim)

    def forward(self, x):
        batch_size = x.size(0)
        K = self.key_compress(x).view(batch_size, self.num_heads, -1)
        V = self.value_compress(x).view(batch_size, self.num_heads, -1)
        attn = F.softmax(K, dim=-1)  # 简化计算
        return self.out_proj((attn * V).reshape(batch_size, -1))


class FusionModel(nn.Module):
    def __init__(self, tcn_params, hidden_size=32, proj_dim=64):  # 增加投影维度
        ...
        self.mla_attn = LiteMLALayer(proj_dim, num_heads=4)
    ...
    def forward(self, x):
        # TCN特征提取
        base_feat = self.tcn(x)
        base_feat = base_feat.transpose(1, 2)  # (batch, seq, 8)

        # 分支特征提取
        lstm_feat = self.lstm_branch(base_feat)  # [B, proj_dim]
        trans_feat = self.trans_branch(base_feat)  # [B, proj_dim]

        # 交叉注意力（LSTM->Transformer）
        cross_feat = self.cross_attn(lstm_feat, trans_feat)

        # MLA自注意力（特征拼接）
        combined = torch.cat([lstm_feat, trans_feat], dim=1)
        mla_feat = self.mla_attn(combined)

        # 门控融合
        gate = self.gate(torch.cat([cross_feat, mla_feat], dim=1))
        fused = gate * cross_feat + (1 - gate) * mla_feat
...
if __name__ == '__main__':
    ...
    # 创建融合模型
    model = FusionModel(tcn_params, hidden_size=64, proj_dim=256)
    pytorch_model = PytorchModel(model)

    # 训练参数
    name = 'TCN_lstm_transformer_CoA256_2_8_128'
    epochs = 150
    batch_size = 256
    lr = 0.001
    patience = 30 # 早停参数
